...
class AutoPatcher:
    ...
    def generate_patch(self, error_type: str, error_location: str,
        ...
        # Only error types with working implementations are dispatched. Anything
        # else is surfaced honestly instead of returning a silent None.
        handlers = {
        ...
        unimplemented = {'override_error', 'type_mismatch', 'null_pointer', 'import_missing', 'resource_not_found'}

        handler = handlers.get(error_type)
        if not handler:
            if error_type in unimplemented:
                logger.warning(f"Patch type '{error_type}' is recognized but not yet implemented; no patch generated.")
            else:
